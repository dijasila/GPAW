def agts(queue):
    gs_N2 = queue.add('gs_N2.py', ncpus=8, walltime=6000)
    ...
    rpa = queue.add('rpa_N2.py', deps=gs_N2, ncpus=16, walltime=3000)
    queue.add('plot_w.py', deps=w, creates='integration.png')
    queue.add('extrapolate.py', deps=rpa, creates='extrapolate.png')

    gs_Kr = queue.add('gs_Kr.py', ncpus=8, walltime=3000)
    rpa = queue.add('rpa_Kr.py', deps=gs_Kr, ncpus=8, walltime=3000)
