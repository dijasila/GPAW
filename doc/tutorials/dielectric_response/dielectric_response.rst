...
along the x-direction, which is the default direction. The file 'df.csv' contain five columns ordered as follows: :math:`\omega` (eV), :math:`\mathrm{Re}(\epsilon_{\mathrm{NLF}})`,
:math:`\mathrm{Im}(\epsilon_{\mathrm{NLF}})`, 
:math:`\mathrm{Re}(\epsilon_{\mathrm{LF}})`,
:math:`\mathrm{Im}(\epsilon_{\mathrm{LF}})` where :math:`\epsilon_{\mathrm{NLF}}` and
:math:`\epsilon_{\mathrm{LF}}` is the result without and with local field
...
The resulting figure is shown below. Note that there is significant absorption close to :math:`\omega=0` because of the large default Fermi-smearing in GPAW. This will be dealt with in the following more realistic calculation.
...
XXX Not anymore. It takes 2 hours with 8 cpus on Intel Xeon X5570 2.93GHz. 
...
XXX Rewrite. Three parallelization schemes over kpoints, spectral_function and frequency 
...
.. autoclass:: gpaw.response.df2.DielectricFunction
