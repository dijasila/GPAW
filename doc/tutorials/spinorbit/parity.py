...
Pso_i = []
for ik, X in zip(range(4), ['G', 'L', 'F', 'Z']):
    ps = get_parity_eigenvalues(calc, ik=ik, spin_orbit=True, bands=range(Nv+10),
                                inversion_center=r_v, deg_tol=1.0e-8)
    P = np.prod(ps[:Nv])
    Pso_i.append(P)
    print('    Parity at %s:' %X, P)
print()
print('Z2 index no spinorbit:', (1 - np.prod(np.array(P_i)**multiplicities)) / 2)
print('Z2 index spinorbit', (1 - np.prod(np.array(Pso_i)**multiplicities)) / 2)
