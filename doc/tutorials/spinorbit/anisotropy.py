...
e_skn = np.array([[calc.get_eigenvalues(kpt=k, spin=s)
                  ...
                  for s in range(2)])
e_kn = np.reshape(np.swapaxes(e_skn, 0, 1), (len(e_skn[0]), 2 * len(e_skn[0,0])))
e_kn = np.sort(e_kn, 1)
f_skn = np.array([[calc.get_occupation_numbers(kpt=k, spin=s)
                  for k in range(len(calc.get_ibz_k_points()))]
                 for s in range(2)])
f_kn = np.reshape(np.swapaxes(f_skn, 0, 1), (len(f_skn[0]), 2 * len(f_skn[0,0])))
...
for theta in theta_i:
    ...
    f = open('anisotropy.dat', 'a')
    E_so = np.sum(e_mk.T * f_km)
    print >> f, theta, E, E_so
    f.close()
