...
Let's do a calculation for `r_s=5` Bohr:

.. literalinclude:: bulk.py

We used a cubic cell with a lattice constant of 1.6 Å, 8*8*8 grid
points and and a k-point sampling of 12*12*12 points.  In the text
output from the calculation, one can see that the cell contains
0.0527907 electrons.
...
Now, we will do a surface calculation.
We put a slab of thickness 12.8 Å inside a box, periodically repeated
in the x- and y-directions only, of size 1.6*1.6*19.2 Å and use 12*12
k-points:
...
which is close to the value from Lang and Kohn: 100
ergs/cm\ `^2`.
