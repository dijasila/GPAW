...
for all the k-points present in he irreducible Brillouin zone from the ground
...
The plane wave cutoff is usually converged by employing a 1/E_cut^{3/2} extrapolation.
This can be done with the following script: :download:`C_ecut_extrap.py` resulting
in a direct band gap of XXX eV. The extrapolation is shown in the figure below
...
number of k-points and frequency sampling results in a direct bandgap of XXX
eV. Hence the value of XXX eV calculated at first was not converged!
...
and 400 eV gives a direct band gap of XXX eV, which is in very good agreement 
with the result for the full frequency integration but the calculation took 
only 1 hour and 35 minutes on 1 xeon-8 CPU!
...
limits for the screened potential: Band gaps of MoS2, h-BN and phosphorene`__
...
:download:`MoS2_gs_GW.py`. The results are not converged but a band gap of XXX eV 
is obtained. 
...
The bands above(below) the highest(lowest) band included will be shifted with the 
k-point average shift of the band below(above). 
...
The calculations are not converged with respect to k-points, frequency points or cutoff energy.
