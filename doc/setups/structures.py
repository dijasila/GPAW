...
if __name__ == '__main__':
    ...
    collections = {'fcc': fcc(), 'rocksalt': rocksalt()}
    for name, collection in collections.items():
        con1 = connect(name + '.db')
        con2 = connect(name + '90.db')
        for symbol in chemical_symbols[1:87]:
            atoms = collection[symbol]
            data = collection.data[symbol]
            atoms.calc = SinglePointCalculator(atoms, energy=data[1])
            atoms.calc.name = 'AIMS'
            con1.write(symbol, atoms)
            atoms.set_cell(atoms.cell * 0.9, scale_atoms=True)
            atoms.calc = SinglePointCalculator(atoms, energy=data[1] + data[6])
            atoms.calc.name = 'AIMS'
            con2.write(symbol, atoms)
