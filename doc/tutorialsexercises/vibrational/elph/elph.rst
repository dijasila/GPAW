...
.. note::
    ...
    The real space grids used in the finite displacement calculations needs to be the same as the one used in the supercell matrix calculation. If you use planewave mode for the finite displacement calculation you should set the required grid manually, for example by adding ``gpts=(nx, ny, nz)`` where ``nx, ny, nz`` need be substituted with the required number of grid points in each direction. This little script might help: :download:`get_best_gpts.py`.
