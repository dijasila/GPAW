...
from ase.units import _hbar, _hplanck, _c, _e, _me, Hartree, Bohr
_eps0_au = 1.0 / (4.0 * np.pi)

from gpaw.fdtd.polarizable_material import PermittivityPlus, Permittivity
...
def plot(fname, fiteps):
    ...
    fiteps_e = np.array([fiteps.value(energy / Hartree) / _eps0_au for energy in energy_e])
