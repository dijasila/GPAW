...
Wannier functions (WFs), the LO construction is not based on a projection
...
for any atom in the system through a subdiagonalization of the Hamiltonian block 
...
(but not among different atoms). Furthermore, the LO representationcan can coexist 
with the original AO one ne, in the sense that one can sub-diagonalize only a subset 
of atoms within the Hamiltonianof the system. This is useful if one is particularly 
