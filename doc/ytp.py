def setup(app):
    ...
    u = "http://gdata.youtube.com/feeds/api/playlists/UUJR-f8bqXzPv5aUlu1-Ba0g?alt=json"
    d = json.loads(requests.get(u).text)
    n = len(d['feed']['entry'])
    i = randint(0, n - 1)
    position = d['feed']['entry'][i]['yt$position']['$t']
    # avoid non-ascii names of people ...
    title = d['feed']['entry'][i]['title']['$t'].split(':')[-1]
    event = d['feed']['entry'][i]['content']['$t'].split('\n')[-1]
    recorded = datetime.strptime(d['feed']['entry'][i]['yt$recorded']['$t'], '%Y-%m-%d').strftime('%d %B %Y')
