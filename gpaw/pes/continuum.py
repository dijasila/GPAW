...
class FirstOrder(ZerothOrder1):
    def get_grid(self, k_c):
        k = sqrt(np.dot(k_c, k_c))
        khat_c = k_c / k

        # get effective potential
        hamiltonian = self.calculator.hamiltonian
        vt_G = hamiltonian.vt_sG[0] # XXX treat spin

        if not hasattr(self, 'intvt'):
            self.intvt = self.gd.integrate(vt_G)

        v1t_G = vt_G # * \hk * \vr
        hamiltonian.poisson.solve(corr1t_G, vt_G, charge=None) 
