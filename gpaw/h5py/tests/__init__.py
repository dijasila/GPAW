...
def runtests(**kwds):
    """ Run low and highlevel gpaw.h5py tests.

    Result is a TestResult object.  Keywords forwarded to TextTestRunner.
    """
    import os, fnmatch
    import gpaw.h5py.tests
    import gpaw.h5py.tests.low
    import gpaw.h5py.tests.high
    ...
    for p in packages:
        ...
        modules = ['gpaw.h5py.tests.%s.%s' % (p, m) for m in files]
        modules = [__import__(m, fromlist=[gpaw.h5py.tests, gpaw.h5py.tests.low, gpaw.h5py.tests.high]) for m in modules]
        for m in modules:
            suite.addTests(ldr.loadTestsFromModule(m))

    # old tests
    files = [x.partition('.py')[0] for x in os.listdir(thisdir) if fnmatch.fnmatch(x, 'test_*.py')]
    modules = ['gpaw.h5py.tests.%s' % m for m in files]
    modules = [__import__(m, fromlist=[gpaw.h5py.tests, gpaw.h5py.tests.low, gpaw.h5py.tests.high]) for m in modules]
    for m in modules:
        suite.addTests(ldr.loadTestsFromModule(m))
