...
def calculate_df(
            ...
            with timer('Sum over bands'):
                for nni in blist:
                    for mmi in blist:
                        ...
                        pnmn = (mom[:, pol[0], nni, mmi]
                                * mom[:, pol[1], mmi, nni]
                                + mom[:, pol[1], nni, mmi]
                                * mom[:, pol[0], mmi, nni])
                        # Factor 4 is due to 2 for area of rectangles and 2 for
                        # pnm
                        Fval = -1j * pnmn * (f_n[:, nni] - f_n[:, mmi]) \
                            / (E_n[:, mmi] - E_n[:, nni]) * dA / 4.0
    ...
    if intmethod == 'no':
        ...
        with timer('Kramer-Kronig relation'):
            ...
            suml = pi * np.imag(suml) / w_l
            interpsum = interp1d(w_l, suml, kind='linear')
            w_lf = np.linspace(w_l[0], w_l[-1], 5000)
            suml2 = interpsum(w_lf)
            suml_b = np.zeros(len(w_l), dtype=np.complex)
            for ind, omega in enumerate(w_lc):
                suml_b[ind] = np.trapz(
                    suml2 * w_lf / (w_lf**2 - omega**2), w_lf)
            chi1 = dim_SI * 2 / pi * suml_b
            chi1 = chi1[:int(nw / 2)]
            # chi1 = dim_SI * suml
...
def calculate_shg_rlg(
    ...
    if intmethod == 'no':
        ...
        for k_c, data_k in k_info.items():
            ...
            # Make the position matrix elements and Delta
            with timer('Position matrix elements calculation'):
                r_nm = np.zeros((3, nb2, nb2), dtype=np.complex)
                D_nm = np.zeros((3, nb2, nb2), dtype=np.complex)
                ...
                rnm_der = np.zeros((3, 3, nb2, nb2), dtype=np.complex)
                for aa in set(pol):
                    for bb in set(pol):
    ...
    chi2b = np.zeros((5, nw), dtype=np.complex)
    ...
    if world.rank == 0:
        if intmethod == 'no':
            ...
            with timer('Kramer-Kronig relation'):
                ...
                suml2A = interpA(w_lf)
                suml2B = interpB(w_lf)
                suml_A = np.zeros(len(w_l), dtype=np.complex)
                suml_B = np.zeros(len(w_l), dtype=np.complex)
                for ind, omega in enumerate(w_lc):
                    suml_A[ind] = np.trapz(
                ...
                for nni in blist:
                    for mmi in blist:
                        ...
                        if np.abs(fnm) > ftol:
                            ...
                            sum2b += 1j * fnm * tmp * w_k[k_c]  # 1j for imag
