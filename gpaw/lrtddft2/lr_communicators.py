    ...
    def initialize(self, calc):
        if self.parent_comm == None:
            ...
            if ( self.parent_comm != self.eh_comm.parent ):
                raise RuntimeError('Invalid communicators in LrTDDFT2. LrTDDFT2 parent communicator does is not parent of electron-hole communicator. Please set up LrCommunicators explicitly to avoid this.')
            if ( self.parent_comm != self.dd_comm.parent ):
                raise RuntimeError('Invalid communicators in LrTDDFT2. LrTDDFT2 parent communicator does is not parent of domain decomposition communicator. Please set up LrCommunicators explicitly to avoid this.')
