    ...
    def __init__(self, ksl, nblocks=None, asynchronous=None, hermitian=None,
        ...
        *roughly* half as much storage space as the non-symmetric case.
        ...
        if self.use_gpu and (self.nblocks > 1 or ngroups > 1):
            raise NotImplementedError('GPU support not implemented for ground '
                                      'state DFT blocking/band parallelization')
        ...
        if B == 1 and J == 1:
            ...
            if gpu.backend.is_device_array(psit_nG):
                ...
                if out_nG is None:
                    ...
                    gpu.backend.memcpy_dtod(work_nG, psit_nG, psit_nG.nbytes)
                    psit_nG = work_nG
                gemm(1.0, psit_nG, gpu.backend.copy_to_device(C_NN), 0.0,
                     out_nG)
            else:
