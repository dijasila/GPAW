...
class PZSICLCAO:
    ...
    def get_hessian(self, kpt, H_MM, n_dim, wfs, setup, C_nM=None,
        ...
        if h_type == 'full':
            ...
            for n in range(n_occ):
                F_MM = self.get_orbital_potential_matrix(
                    f_n, C_nM, kpt, wfs, setup, n, wfs.timer)[0]
                F_MM += H_MM
            ...
            L_unocc = np.einsum(
                'jk,kl,il->ji', C_nM.conj()[n_occ:], H_MM.conj(), C_nM[n_occ:])
            L_unocc = 0.5 * (L_unocc + L_unocc.T.conj())
    ...
    def get_energy_and_gradients_inner_loop(self, wfs, kpt, a_mat,
        ...
        e_total_sic = np.array([])
        for n in range(n_occ):
            F_MM, sic_energy_n = self.get_orbital_potential_matrix(
                kpt.f_n, kpt.C_nM, kpt, wfs, wfs.setups, n, wfs.timer)
