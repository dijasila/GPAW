...
class SPzCorrectionsLcao2:
    ...
    def get_energy_and_gradients(self, wfs, dens, grad_knG, U_k=None):
        ...
        for kpt in wfs.kpt_u:
            ...
            for i in range(n_occ):
                # TODO: fix sum v_scom
                grad_s[kpt.s][i] += kpt.psit_nG[i] * (self.v_scom.sum(axis=0))
                c_axi = {}
                for a in kpt.P_ani.keys():
