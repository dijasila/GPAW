...
from gpaw.utilities import packed_index, unpack_h, unpack_sum
...
class HF:
        ...
        for n1 in range(self.nbands):
            ...
            for n2 in range(n1, self.nbands):
                ...
                for a, v_L in v_aL.items():
                    v_ii = unpack_h(np.dot(paw.wfs.setups[a].Delta_pL, v_L))
                    P_ni = kpt.P_ani[a]
    ...
    def atomic_val_val(self, paw, H_nn, u=0):
        ...
        for a, P_ni in kpt.P_ani.items():
            ...
            H_nn += np.dot(P_ni, np.inner(unpack_h(H_p), P_ni.conj()))

    def atomic_val_core(self, paw, H_nn, u=0):
        kpt = paw.wfs.kpt_u[u]
        for a, P_ni in kpt.P_ani.items():
            dH_ii = unpack_h(-paw.wfs.setups[a].X_p)
