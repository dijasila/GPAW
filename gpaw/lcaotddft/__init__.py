...
def print_matrix(M, file=None, rank=0):
    ...
    if world.rank == 0:
        ...
        for i in range(a):
            for j in range(b):
                print('%.7f' % M[i][j].real, '%.7f' % M[i][j].imag,
                      end=' ', file=f)
            print(file=f)
        if file is not None:
            f.close()
        ...
        if self.propagator_debug:
            if self.blacs:
                ...
                if world.rank == 0:
                    ...
                    U_MM = dot(inv(globalS_MM - 0.5j * globalH_MM * dt),
                               globalS_MM + 0.5j * globalH_MM * dt)
            ...
            if self.propagator == 'taylor' and self.blacs:
                ...
                if self.propagator_debug:
                    ...
                    for kpt in self.wfs.kpt_u:
                        ...
                        self.mm2MM.redistribute(self.wfs.S_qMM[kpt.q],
                                                invS2_MM)
                        world.barrier()
                        if world.rank == 0:
                            tri2full(invS2_MM, 'L')
                        ...
                        verify(kpt.invS_MM, kpt.invS2_MM,
                               'overlap par. vs. serial', 'L')
