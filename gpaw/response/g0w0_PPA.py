...
class G0W0(PairDensity):
    ...
    def calculate_q(self, i, kpt1, kpt2):
        ...
        if self.ppa:
            W_wGG, self.wt_GG = [np.load(fd).take(G_G, 0).take(G_G, 1) for x in self.omega_w]
        else:
    ...
    def calculate_sigma(self, fd, n_mG, deps_m, f_m, W_wGG):
        ...
        assert self.alpha == 0
        if self.ppa:
            x1_mGG = 1 / (deps_m[:, np.newaxis, np.newaxis] + self.wt_GG - 1j * self.eta)
            x2_mGG = 1 / (deps_m[:, np.newaxis, np.newaxis] - self.wt_GG + 1j * self.eta)
            x3_mGG = 1 / (deps_m[:, np.newaxis, np.newaxis] + self.wt_GG - 2j * self.eta * (f_m[:, np.newaxis, np.newaxis] - 0.5))
            x4_mGG = 1 / (deps_m[:, np.newaxis, np.newaxis] - self.wt_GG - 2j * self.eta * (f_m[:, np.newaxis, np.newaxis] - 0.5))
            x_mGG = W_wGG * (2 * (f_m[:, np.newaxis, np.newaxis] - 0.5) * (x1_mGG - x2_mGG) + (x3_mGG + x4_mGG))
            dx_mGG = W_wGG * (2 * (f_m[:, np.newaxis, np.newaxis] - 0.5) * (x1_mGG**2 - x2_mGG**2) + (x3_mGG**2 + x4_mGG**2))
...
#            print(chi0_wGG.shape, file=self.fd)
            ...
            if self.ppa:
                einv1_GG = np.linalg.inv(delta_GG - 4 * pi * chi0_wGG[0] * iG_G * iG_G[:, np.newaxis]) - delta_GG
                einv2_GG = np.linalg.inv(delta_GG - 4 * pi * chi0_wGG[1] * iG_G * iG_G[:, np.newaxis]) - delta_GG
                wt_GG = self.E0 * np.sqrt(einv2_GG / (einv1_GG - einv2_GG))
                R_GG = - wt_GG / 2 * einv1_GG
