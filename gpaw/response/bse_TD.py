...
class BSEBackend:
    ...
    def diagonalize(self):
        ...
        if not self.td:
           ...
           # self.rhoG0_S = np.delete(self.rhoG0_S, self.excludef_S, axis = 1)
           # self.rhoG0_S = np.delete(self.rhoG0_S, self.excludef_S, axis = 2)
           # self.rhoG0_S = np.reshape(self.rhoG0_S, (-1, nG, nG))
    ...
    def get_vchi(self, w_w=None, eta=0.1, q_c=[0.0, 0.0, 0.0],
                 ...
                 write_eig=None, return_vchi = True):
        ...
        if not self.td:
            if world.rank == 0:
               ...
               # C_TGG1 = A_GT.T.conj()[...,np.newaxis] * ((B_GT@overlap_tt.T.conj()).T)[:,np.newaxis]
	...
	"""
       ...
       # if not return_vchi:
       #         sqrtV_G = self.v_G**(1/2)
       #         vchi_w /= sqrtV_G * sqrtV_G[:, np.newaxis]
