...
class PAWPairDensityCalculator:
    def __init__(self, pawdata, radial_points=None):
        ...
        gcut2 = self.rgd.ceil(2 * max(pawdata.rcut_j))
        ...
        self.dn_jjg = (phi_jg[j_j, None] * phi_jg[None, j_j]
                       - phit_jg[j_j, None] * phit_jg[None, j_j])
        for j1, l1 in enumerate(self.l_j):
            ksplines_jl = list()

            for j2, l2 in enumerate(self.l_j):
                dn_g = self.dn_jjg[j1, j2]
                ksplines_l = list()

                for j, l in enumerate(range(abs(l1 - l2), l1 + l2 + 1, 2)):
                    # To evaluate the radial integral efficiently, we rely on
                    # the Fast Fourier Bessel Transform (FFBT) algorithm, see
                    # gpaw.ffbt In order to do so, we make a spline
                    # representation of the radial partial wave correction
                    # rescaled with a factor of r^-l

                    spline = self.rgd.spline(dn_g[:gcut2], l=l,
                                             points=radial_points)

                    # This allows us to calculate a spline representation of
                    # the spherical Fourier-Bessel transform
                    #                 rc
                    #             4π  /
                    ...
                    ksplines_l.append(kspline)
                ksplines_jl.append(ksplines_l)
            ksplines_jjl.append(ksplines_jl)
        self.kspline_cache = ksplines_jjl

    def __call__(self, qG_Gv):
        r"""Calculate the atom-centered PAW correction to the pair density.
                                                        ˍ
        The atom-centered pair density correction tensor, Q_aii', is defined as
        the atom-centered Fourier transform

        ˍ             /                                     ˷         ˷
        Q_aii'(G+q) = | dr e^-i(G+q)r [φ_ai^*(r) φ_ai'(r) - φ_ai^*(r) φ_ai'(r)]
                    /

        evaluated with the augmentation sphere center at the origin. The full
        pair density correction tensor is then given by
                                    ˍ
        ...
        for j1, l1 in enumerate(l_j):
            ...
            for j2, l2 in enumerate(l_j):
                ...
                # Sample l according to the Gaunt coefficient selection rules,
                # see e.g. gpaw.test.test_gaunt
                for j, l in enumerate(range(abs(l1 - l2), l1 + l2 + 1, 2)):
                    kspline = self.kspline_cache[j1][j2][j]

                    # Now, this implementation relies on a range of hardcoded
                    # values, which are not guaranteed to work for all cases.
                    # In particular, the uniform radial grid used for the FFBT
                    # is defined through the `rcut` and `N` parameters in the
                    # `rescaled_fourier_bessel_transform()` function (the
                    # former is currently hardcoded), and the `points`
                    # parameter to `rgd.spline()` controls an intermediate
                    # interpolation step.
                    # To make a generally reliable implementation, one would
                    # need to control these parameters based on the setup,
                    # e.g. the nonlinear radial grid spacing. In doing so,
                    # one should be mindful that the rcut parameter defines
                    # the reciprocal grid spacing of the kspline representation
                    # and that N controls the k-range (which might need to
                    # depend on input qG_Gv).

                    # For now, we simply check that the requested plane waves
                    # are within the computed k-range of the FFBT and check
                    # that the resulting transforms match a manual calculation
                    # at a few selected K-vectors.
                    kmax = np.max(k_G)
                    assert kmax <= kspline.get_cutoff()
                    # Manual calculation at kmax
                    dnmax = rgd.integrate(spherical_jn(l, kmax * rgd.r_g)
                                          * dn_g)
                    # Manual calculation at average k
                    kavg = np.average(k_G)
                    dnavg = rgd.integrate(spherical_jn(l, kavg * rgd.r_g)
                                          * dn_g)
                    k_k = [kmax, kavg]
                    dn_k = [dnmax, dnavg]
                    if l == 0:
                    ...
                    for m in range(l**2, (l + 1)**2):
                        ...
                        for m1 in range(2 * l1 + 1):
                            for m2 in range(2 * l2 + 1):
                                # Set up the i=(l,m) index for each
                                # partial wave
                                i1 = i1_counter + m1
                                i2 = i2_counter + m2
                                # Extract Gaunt coefficients
                                gaunt_coeff = self.G_LLL[l1**2 + m1,
                                                         l2**2 + m2, m]
                                if (gaunt_coeff == 0).all():
                                    continue

                                # Add contribution to the PAW correction
                                Qbar_Gii[:, i1, i2] += (gaunt_coeff * klY_G
                                                        * f_G)
