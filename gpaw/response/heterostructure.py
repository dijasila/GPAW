...
class Heterostructure:
    ...
    def CoulombKernel(self, q_abs, full=True):
        ...
        def v_dd(q, d): # ????
            delta = 4 / Bohr
            V_p = 1. / q * np.exp(-q * d)
            V_m = -1. / q * np.exp(-q * (np.abs(d) + delta))
            temp = 2 * np.pi * (V_p + V_m)
            return temp
	...
	"""Dyson like equation for \chi: \chi_full = chi_intra + chi_intra V_inter chi_full"""
        ...
        if self.chi_dipole is not None:
            ...
            for iq in range(len(q_points_abs)):
                kernel_ij = self.CoulombKernel(q_abs = self.q_points_abs[iq], 
