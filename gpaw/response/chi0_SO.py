...
class Chi0_SO(Chi0):
    ...
    def __init__(self, calc, *args, **kwargs):
        ...
        e_mk, v_knm = self.calc_SO(socalc)
        ...
        kwargs.pop('disable_time_reversal')
        Chi0.__init__(self, calc, omegamax=omegamax,
                      disable_time_reversal=True,
                      *args, **kwargs)

        self.pair.fermi_level = socalc.occupations.fermilevel
        self.ftol = self.pair.ftol
        self.e_mk = e_mk
        self.v_knm = v_knm
        self.count_occupied_bands()

    def calc_SO(self, socalc):
        return get_spinorbit_eigenvalues(socalc, return_wfs=True)
    ...
    def count_occupied_bands(self):
        self.nocc1 = 9999999
        self.nocc2 = 0
        for kpt in self.socalc.wfs.kpt_u:
    ...
    def get_eigenvalues(self, k_v, *args, **kwargs):
        ...
        e1_m = self.e_mk[self.nocc2:, ik1] / Hartree
        e2_m = self.e_mk[:self.nocc1, ik2] / Hartree
        deps_nm = np.subtract(e1_m[:, np.newaxis], e2_m)

        return deps_nm.reshape(-1)

    @timer('get_matrix_element')
    def get_matrix_element(self, k_v, *args, **kwargs):
        ...
        v1_nm = self.v_knm[ik1][:self.nocc2]
        v2_nm = self.v_knm[ik2][self.nocc1:]
        ...
        #     print('get matrix element')
        with self.timer('Chi0.get_matrix_element'):
        ...
        f1_n = kpt1.f_n[:self.nocc2] / kpt1.weight
        f2_n = kpt2.f_n[self.nocc1:] / kpt2.weight
        ...
        with self.timer('Matrix products'):
            # v1.n.v2.T
            n_nmG = np.dot(v1_nm.conj(), np.dot(v2_nm, n_nmG))
            # mmm(1.0, n_nmG, 'T', v2_nm, 'T', 0.0, n_nmG)  # -> n_mnG
            # mmm(1.0, n_nmG, 'T', v1_nm, 'C', 0.0, n_nmG)
        # if self.world.rank == 0:
        #     print('    done')
    ...
    def get_intraband_response(self, k_v, *args, **kwargs):
        ...
        K1 = self.pair.find_kpoint(k_c)
        ik1 = kd.bz2ibz_k[K1]
        v1_nm = self.v_knm[ik1][self.nocc1:self.nocc2]

        nb = self.nbands
        opd = self.pair.optical_pair_velocity

        vel_nmv = np.zeros((2 * nb, 2 * nb, 3), dtype=complex)
    ...
    def get_intraband_eigenvalue(self, k_v, *args, **kwargs):
        ...
        return self.e_mk[self.nocc1:self.nocc2, ik1] / Hartree
