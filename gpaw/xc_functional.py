...
###            self.xcname = xcname # MDTMP: to get the xcname name for setup
    ...
    def get_non_local_energy_and_potential1D(self, gd, u_j, f_j, e_j, l_j, v_xc, iteration, density=None):
        # Send the command one .xc up
        return self.xc.get_non_local_energy_and_potential1D(gd, u_j, f_j, e_j, l_j, v_xc, iteration, 
                                                            density=density)
...
class XCRadialGrid(XCGrid):
    ...
    def get_non_local_energy_and_potential(self, u_j, f_j, e_j, l_j, v_xc, iteration, density=None):
