...
def test_dipole_me(gpw_files):
    ...
    """assert abs(d2_nnv[0, 0] - calc.atoms.cell.sum(0) / 2).max() < 1e-5
    assert abs(d2_nnv[1, 1] - calc.atoms.cell.sum(0) / 2).max() < 1e-5
    assert abs(d2_nnv[0, 1] - d1_v).max() < 1e-5
    """
    ...
    print(d2_nnv)
    print(d3_nnv)
    assert abs(d3_nnv[0, 0]).max() < 1e-5
    assert abs(d3_nnv[1, 1]).max() < 1e-5
    assert abs(d3_nnv[0, 1] - d2_nnv[0, 1]).max() < 1e-9
