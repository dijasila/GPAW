...
equal(np.linalg.norm(wan.get_centers() - [com, com]), 0, 2e-4)

wan = Wannier(nwannier=2, calc=calc, initialwannier='projectors')
equal(wan.get_functional_value(), 3.100, 1e-2)
equal(np.linalg.norm(wan.get_centers() - pos), 0, 1e-2)
...
equal(wan.get_functional_value(), 3.100, 1e-2)
equal(np.linalg.norm(wan.get_centers() - pos), 0, 1e-2)
equal(np.linalg.norm(wan.get_radii() - 1.2393), 0, 1e-2)
