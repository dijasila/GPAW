...
def mark_xfail(gs, request):
    if gs == 'al_pw' or gs == 'fe_pw' or gs == 'co_pw':
...
def test_ibz2bz(in_tmp_dir, gpw_files, gs, only_ibz_k, request):
    ...
    def find_degenerate_subspace(eps, i, nbands):
        ...
        j = i
        dim = 1
        while j < nbands - 1 and abs(eps[j] - eps[j + 1]) < atol_eig:
            dim += 1
            j += 1
        return dim
    ...
    for s in range(wfs.nspins):
        for K in range(nbzk):
            ...
            # if only_ibz_k fixture is true only test ibz k-points
            if only_ibz_k and not np.allclose(wfs.kd.bzk_kc[K],
                                              wfs.kd.ibzk_kc[ik]):
                continue
            ...
            assert np.allclose(np.array(dO_aii),
                               np.array(get_overlaps(wfs_nosym)))
