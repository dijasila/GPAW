...
def test_directmin_lcao_numerical_hessian(in_tmp_dir):
    ...
    n_dim = {}
    n_dim[0] = calc.wfs.bd.nbands
    hess_a, hess_n = calc.wfs.eigensolver.get_numerical_hessian(n_dim,
                                                                calc.hamiltonian,
                                                                calc.wfs,
                                                                calc.density,
    ...
    calc.wfs.eigensolver.rotate_wavefunctions(calc.wfs,
                                              ...
                                              n_dim,
                                              calc.wfs.eigensolver.c_nm_ref)
    c_nm = [calc.wfs.kpt_u[x].C_nM.copy() for x in range(len(calc.wfs.kpt_u))]
    hess_a, hess_n = calc.wfs.eigensolver.get_numerical_hessian(n_dim,
                                                                calc.hamiltonian,
                                                                calc.wfs,
                                                                calc.density,
