...
v_nG = pd.zeros(nb)
VV_aii = [np.zeros((1, 1)) + 0.0]
psit.matrix_elements(pt, out=proj)
x = xx.calculate_energy([kpt], VV_aii, [v_nG])
v = v_nG[0, 1]
print(v_nG)
print(x)
eps = 0.001
data[0, 1] = 1 + eps
psit.matrix_elements(pt, out=proj)
xp = xx.calculate_energy([kpt], VV_aii, [v_nG])
data[0, 1] = 1 - eps
psit.matrix_elements(pt, out=proj)
xm = xx.calculate_energy([kpt], VV_aii, [v_nG])
d = (xp[0] - xm[0]) / (2 * eps) * N**3 * 4
print(v / d)
