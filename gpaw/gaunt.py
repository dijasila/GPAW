...
def make_ylnyl(lmax=2):
    ...
    # Try to load old ylnyl matrix
    try:
        from gpaw.gaunt import ylnyl
    except ImportError:
        ylnyl = npy.zeros((0, 0, 0))
        
    # Don't recalculate if already present
    if Lmax <= ylnyl.shape[0]:
        return ylnyl[:Lmax, :Lmax, :L2max]

    # More element needed, setup new matrix
    from gpaw.spherical_harmonics import YL, gam
    old = ylnyl
    del ylnyl
    ylnyl = npy.zeros((Lmax, Lmax, L2max))

    # Insert old values
    ylnyl[:old.shape[0], :old.shape[1], :old.shape[2]] = old
...
if __name__ == '__main__':
    ...
    gaunt = make_gaunt(2)
    print 'gaunt = npy.array(%s)' % gaunt.tolist()
