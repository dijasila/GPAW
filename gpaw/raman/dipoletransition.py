...
def get_dipole_transitions(atoms, calc):
    """
    ...
    for s in range(nspins):
        ...
        for k in range(nk):
            ...
            wf = np.array([calc.wfs.get_wave_function_array(i, k, s, realspace = True, periodic=True) for i in range(n)], dtype = complex)
        ...
        for k, (wf, P_nI) in wfs.items():
            ...
            k_v = np.dot(calc.wfs.kd.ibzk_kc[k],calc.wfs.gd.icell_cv) * 2 * np.pi
