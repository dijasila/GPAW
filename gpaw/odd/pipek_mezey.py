...
def md_min(func, step=.25, tolerance=1e-6, verbose=False, gd=None, **kwargs):
    if gd is not None:
        if gd.comm.rank == 0:
            if verbose:
...
class PipekMezey:
    ...
    def __init__(self, calc, method='H', ftol=1e-8,
        ...
        self.W_k = np.zeros((self.Nk, self.nocc, self.nocc), dtype=self.dtype)
        ...
        for d in range(self.Nd):
            for k1 in range(self.Nk):
                self.invlst_dk[d, k1] = self.lst_dk[d].tolist().index(k1)
        ...
        for atom in self.atoms:
            ...
            #
                ...
                self.WFa[atom.index] = WignerSeitz(self.gd, self.atoms,
                                        atom.index).construct_weight_function()
        # Using WFa and k-d lists make overlap matrix
        Qadk_nm = np.zeros((self.Na,self.Nd,self.Nk,self.nocc,self.nocc), complex)
        #
            ...
            for k in range(self.Nk):
                ...
                for n in range(self.nocc):
                    self.wfs.kpt_u[k].psit_nG[n] = \
                        self.wfs._get_wave_function_array(k, n, periodic=True)
        #
        #
        for d, dG in enumerate(self.Gd):
            ...
            for k in range(self.Nk):
                ...
                # Inner product
                e_G = np.exp(-2j * pi * np.dot(np.indices(self.gd.n_c).T +
    ...
    def update_matrices(self):
        ...
        for a in range(self.Na):
            for d in range(self.Nd):
                for k in range(self.Nk):
                    k1 = self.lst_dk[d, k]
                    self.Qadk_nn[a,d,k] = np.dot(self.W_k[k].T.conj(), np.dot(
