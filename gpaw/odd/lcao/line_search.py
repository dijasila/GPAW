...
class Parabola:
    ...
    def step_length_update(self, A_s, P_s, n_dim,
                           phi_0=None, der_phi_0=None,
                           phi_old=None, der_phi_old=None,
                           alpha_max=10.0, alpha_old=None):

        phi_max, der_phi_max, g_max = \
            self.evaluate_phi_and_der_phi(A_s, P_s, n_dim,
                                          alpha_max)

        a_star = parabola_interpolation(0.0, alpha_max,
                                       phi_0, phi_max,
                                       der_phi_0)
...
class Parabola_outer:
    ...
    def step_length_update(self, x_k, p_k, n_dim,
                           phi_0=None, der_phi_0=None,
                           phi_old=None, der_phi_old=None,
                           alpha_max=1.0, alpha_old=1.0):
        ...
        phi_max, der_phi_max, g_max = \
            self.evaluate_phi_and_der_phi(x_k, p_k, n_dim,
                                          alpha_max)
        ...
        phi_star, der_phi_star, g_star = \
            self.evaluate_phi_and_der_phi(x_k, p_k, n_dim,
                                          a_star, perf_orth=True)

        return a_star, phi_star, der_phi_star, g_star

    def init_guess(self, phi_0, der_phi_0, phi_old, der_phi_old,
                   alpha_old=1.0, method='1'):
        ...
        if method == '1':
            ...
            if der_phi_0 is not None and der_phi_old is not None:
                try:
                    alpha_1 = alpha_old * der_phi_old / der_phi_0
                except ZeroDivisionError:
                    alpha_1 = 1.0
...
class StrongWolfeConditions_outer:
    """
    From a book of Jorge Nocedal and Stephen J. Wright 'Numerical
    Optimization' Second Edition, 2006 (p. 56)
    ...
    def __init__(self, evaluate_phi_and_der_phi, log,
                 c1=1.0e-4, c2=0.9,
                 method=None, max_iter=10, eps_dx=1.0e-10,
                 eps_df=1.0e-10, awc=True):
        """
        :param evaluate_phi_and_der_phi: function which calculate
        phi, der_phi and g for given A_s, P_s, n_dim and alpha
    ...
    def step_length_update(self, x_k, p_k, n_dim,
        ...
        while True:
            ...
            if phi_i > (1.0) * phi_0 + c1 * alpha[i] * \
                    der_phi_0 or \
                    (phi_i >= (1.0) * phi_i_1 and i > 1):
                # parprint('Went to ZOOM I')
                # parprint(phi_i, (1.0 - 1.0e-5) * phi_0 + c1 *
                # alpha[i]
                #      * \
                #    der_phi_0, alpha[i])
                a_star, phi_star, der_phi_star, g_star = \
            ...
            # parprint(alpha[-1], alpha[-2])
            # if i == max_iter:
            if abs(alpha[-1] - alpha[-2]) < 1.0e-5:
                # print 'in get step length'
                self.log('Cannot satisfy strong Wolfe condition')
                # self.log(alpha[-1] - alpha[-2])
                self.log('Too much iterations')
    ...
    def zoom(self, a_lo, a_hi,
             f_lo, df_lo, f_hi, df_hi, x_k,
             p_k, n_dim, phi_0, der_phi_0,
             c1, c2):
        ...
        while True:
            ...
            phi_j, der_phi_j, g_j = \
                self.evaluate_phi_and_der_phi(x_k, p_k, n_dim, a_j)
            ...
            if phi_j > (1.0) * phi_0 + c1 * a_j * der_phi_0 \
                    or phi_j \
                    >= (1.0) * f_lo:
                # print('I \n')
                a_hi = a_j
            ...
            # if i == max_iter:
            #     self.log('Cannot satisfy strong Wolfe condition,')
            #     self.log('Made too much iterations')
            #     if a_lo > self.eps_dx:
            #         a_star = a_lo
            #
            #         phi_star, der_phi_star, g_star = \
