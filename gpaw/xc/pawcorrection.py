...
class PAWXCCorrection:
    ...
    def calculate(self, xc, D_sp, dH_sp):
        if xc.name == 'GLLB':
        ...
        sign = 1
        for n_qg, nc_g, tau_pg, tauc_g in [
            (self.n_qg, self.nc_g, self.tau_pg, self.tauc_g),
            (self.nt_qg, self.nct_g, self.taut_pg, self.tauct_g)]:
