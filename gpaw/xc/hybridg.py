...
class HybridXC(HybridXCBase):
    ...
    def initialize(self, dens, ham, wfs, occupations):
        ...
        self.pd = PWDescriptor(wfs.pd.ecut, wfs.gd, kd=self.kd)
        self.pd2 = PWDescriptor(dens.pd2.ecut, dens.gd, kd=self.qd)
        self.pd3 = PWDescriptor(dens.pd3.ecut, dens.finegd, kd=self.qd)
        #self.pd3 = self.pd2
        ...
        self.ghat = PWLFC([setup.ghat_l for setup in dens.setups], self.pd3)
        
        self.log('Value of alpha parameter:', self.alpha)
        self.log('Value of gamma parameter:', self.gamma)
        self.log('Cutoff energies:')
        self.log('    Wave functions:', self.pd.ecut, 'Hartree')
        self.log('    Density:', self.pd2.ecut, 'Hartree')
        self.log('    Compensation charges:', self.pd3.ecut, 'Hartree')
    ...
    def apply(self, kpt1, kpt2, k):
        ...
        for n1 in range(self.nbands):
            ...
            for n2 in range(self.nbands):
                ...
                t0 = time()
                nt_G = self.calculate_pair_density(n1, n2, kpt1, kpt2, q, k,
                                                   eik20r_R, eik2r_R, G3_G, f_IG)
                vt_G = nt_G.copy()
                vt_G *= -pi * iG2_G
                e = self.pd3.integrate(nt_G, vt_G).real * nspins * self.hybrid * x
                if self.etotflag:
                    if self.acdf:
                        ...
                        self.exx += f2 * e * self.kd.weight_k[kpt1.k] * f1 * self.kd.nbzkpts
                else:
                ...
                if is_ibz2:
                    if self.etotflag:
                        if self.acdf:
                            ...
                            self.exx += f1 * e * self.kd.weight_k[kpt2.k] * f2 * self.kd.nbzkpts
                    else:
    ...
    def calculate_pair_density(self, n1, n2, kpt1, kpt2, q, k,
                               eik20r_R, eik2r_R, G3_G, f_IG):
        u1_R = self.pd.ifft(kpt1.psit_nG[n1], kpt1.k)
        u2_R = self.pd.ifft(kpt2.psit_nG[n2], kpt2.k)
        psit2_R = u2_R * eik20r_R
        psit2_R = self.kd.transform_wave_function(psit2_R, k)

        nt_R = u1_R.conj() * psit2_R / eik2r_R
        #nt_R = u1_R.conj() * u2_R
        if self.pd2 is self.pd3:
            nt_G = self.pd2.fft(nt_R, q)
        else:
            nt_G = self.pd3.zeros(q=q)
        ...
        self.ghat.add(nt_G, Q_aL, q, f_IG)
        return nt_G
