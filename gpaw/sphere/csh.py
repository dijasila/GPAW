...
def intYY(l1, m1, l2, m2):
    ...
    """
    if (l1, m1) == (l2, m2):
        return C(l1, m1)**2 * 2 * np.pi * ilegendre(
            l1, m1)  # == 1 always, XXX right?
    else:
        return 0.0
...
def intYY_ez(l1, m1, l2, m2):
    ...
    return scale * (l2 - np.sign(l1 - l2) * abs(m1) +
...
def intYdYdphi_ex(l1, m1, l2, m2):
    ...
    if abs(m1) == abs(
            m2) + 1 and l1 > l2:  # and (l1-l2)%2 == 1 which is always true
        return scale * 2 * lmfact(l2, m2)
    elif abs(m1) == abs(
            m2) - 1 and l1 < l2:  # and (l2-l1)%2 == 1 which is always true
