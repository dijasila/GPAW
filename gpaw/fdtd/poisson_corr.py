...
class PoissonSolver:
    ...
    def solve(self, phi, rho, charge=None, eps=None, maxcharge=1e-6,
              zero_initial_phi=False, remove_moment=None):
        ...
        master = self.gd.comm.rank==0
        ascale = 25.0 #/self.gd.get_grid_spacings()[0]
        
        moments = []
        
        if remove_moment and not self.gd.pbc_c.any():
            rho_neutral = rho.copy()
            phi_cor_L = []
            for L in range(remove_moment):
                moments.append(Gaussian(self.gd).get_moment(rho_neutral, L)) #ascale?
                phi_cor_L.append(Gaussian(self.gd).remove_moment(rho_neutral, L)) #ascale?
