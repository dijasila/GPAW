...
# NOTE: This class is used to encapsulate basis functions with different
# cutoffs. This will probably be reimplemented in C some day
#
# Concerns only lcao calculations so far
#
# Some methods were apparently not required for lcao calculations
# and have therefore not been tested properly
class DissimilarlyLocalizedFunctions:
    """Localized functions with different cutoffs.

    This class implements the localized functions interface, but
    will internally use separate LocFuncs objects to represent
    functions with different cutoffs, thus saving memory."""
    def __init__(self, dissimilarly_localized_functions):
        """Create DissimilarlyLocalizedFunctions object from a list
        of LocFuncs objects with differing cutoffs."""
        self.lf_j = tuple(dissimilarly_localized_functions)
        self.ni_j = [lf.ni for lf in self.lf_j]
        i_j = [] # Indices for each LocFuncs object
        ni = 0
        for lf in self.lf_j:
            i_j.append(ni)
            ni += lf.ni
        self.ni = ni # Total number of comprised localized functions

        # Index slices for each locfuncs object
        self.slice_j = [npy.arange(i, i + ni) for i, ni in zip(i_j, self.ni_j)]

    def iterchain(self, iterators):
        while True:
        ...
        for i in range(steps):
            for iter in iterators:
                iter.next()
            # Actually we could probably just run next() on all of them,
            # eventually resulting in StopIteration at the right time
            # Then we can just use while True: instead of counting steps
            print 'i=%d' % i
            yield None

    def itercollect(self, iterator, dst_xi, src_jxI, steps=3):
        for i in range(steps - 1):
            yield iterator.next()
        iterator.next()
        self.collect(dst_xi, src_jxI)
        yield None
        
    def distribute(self, objs_xi):
        """Helper method to split up arrays between localized functions."""
        return [npy.array(objs_xi[:, slice], order='C')
                for slice in self.slice_j]

    def distribute0(self, objs):
        """Split up 1d-arrays between localized functions."""
        return [npy.array(objs[slice], order='C') for slice in self.slice_j]

    def collect(self, dst_xi, src_jxI):
        """Join arrays previously distributed among localized functions."""
        for slice, src_xI in zip(self.slice_j, src_jxI):
            dst_xi[:, slice] = src_xI

    def zdistribute(self, objs_xi):
        """Like distribute, but zips list with localized functions"""
        return zip(self.lf_j, self.distribute(objs_xi))

    def zdistribute0(self, objs_i):
        """Like distribute0, but zips list with localized functions"""
        return zip(self.lf_j, self.distribute0(objs_i))
    ...
    def iadd(self, a_xg, coef_xi, k=None, communicate=False):
        for x in self.iterchain([lf.iadd(a_xg, coefchunk_xI, k, communicate)
                                 for lf, coefchunk_xI
                                 in self.zdistribute(coef_xi)]):
            yield None
    ...
    def iintegrate(self, a_xg, result_xi, k=None):
        raise RuntimeError('untested method')
        result_jxI = self.distribute(result_xi)
        chain = self.iterchain([lf.iintegrate(a_xg, resultchunk_xi, k)
                                for lf, resultchunk_xi
                                in zip(self.lf_j, result_jxI)])
        return self.itercollect(chain, result_xi, result_jxI)
    ...
    def isum(self, a_x, broadcast=False):
        raise RuntimeError('untested method')
        return self.iterchain([lf.isum(a_x, broadcast)
                               for lf in self.lf_j])
    ...
    def iderivative(self, a_xg, result_xic, k=None):
        raise RuntimeError('untested/unimplemented method')
        return self.iterchain([lf.iderivative(a_xg, resultchunk_xic, k)
                               for lf, resultchunk_xic
                               in self.zdistribute(resultchunk_xic, -2)])

    def add_density(self, n_G, f_i):
        for lf, fchunk_i in self.zdistribute0(f_i):
            lf.add_density(n_G, fchunk_i)
        
        
    def add_density2(self, n_G, D_p):
        raise RuntimeError('untested method')
        return sum([lf.add_density2(n_G, D_p)
                    for lf in self.lf_j])

    def norm(self):
        raise RuntimeError('untested method')
        return npy.concatenate([lf.norm() for lf in self.lf_j])
