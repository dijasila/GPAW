...
class Matrix:
    ...
    def array(self, data):
        if data is None:
            ...
            self._array_gpu = None
            if self.use_gpu:
                self._array_gpu = gpu.backend.copy_to_device(self._array_cpu)
    ...
    def sync(self):
        try:
            if self.on_gpu:
                gpu.backend.copy_to_host(self._array_gpu, self._array_cpu)
            else:
                gpu.backend.copy_to_device(self._array_cpu, self._array_gpu)
