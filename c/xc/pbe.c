  ...
  if (gga)
    {
      ...
      double A;
      if (x != 1.0)
        A = BETAPBESOL / (GAMMA * (x - 1.0)); 
      else
        A = BETAPBESOL / (GAMMA * y);
      double At2 = A * t2;
      double nom = 1.0 + At2;
      double denom = nom + At2 * At2;
      double H = GAMMA * log( 1.0 + BETAPBESOL * t2 * nom / (denom * GAMMA));
      double tmp = (GAMMA * BETAPBESOL /
                    (denom * (BETAPBESOL * t2 * nom + GAMMA * denom)));
      double tmp2 = A * A * x / BETAPBESOL;
      double dAdrs = tmp2 * *dedrs;
      if (spinpol)
        {
          H *= phi3;
      ...
      double dHdt2 = (1.0 + 2.0 * At2) * tmp;
      double dHdA = -At2 * t2 * t2 * (2.0 + At2) * tmp;
      *dedrs += (dHdt2 * 7 * t2 / rs + dHdA * dAdrs) * pbesolcfrac;
      *deda2 = dHdt2 * C3 * rs / n2 * pbesolcfrac;
      if (spinpol)
        {
          double dphidzeta = (1.0 / xp - 1.0 / xm) / 3.0;
          double dAdzeta = tmp2 * (*dedzeta - 
