...
PyObject* Transformer_apply_cuda_gpu(TransformerObject *self, PyObject *args)
{
    ...
    for (int n = 0; n < nin; n += blocks) {
...
#ifdef DEBUG_CUDA_TRANSFORMER
        GPAW_CUDAMEMCPY(in_cpu, in2, double, ng * myblocks,
                        cudaMemcpyDeviceToHost);
        GPAW_CUDAMEMCPY(out_cpu, out2, double, out_ng * myblocks,
                        cudaMemcpyDeviceToHost);
#endif //DEBUG_CUDA_TRANSFORMER
        if (self->interpolate) {
...
#ifdef DEBUG_CUDA_TRANSFORMER
            GPAW_CUDAMEMCPY(buf_cpu2, transformer_buf_gpu, double,
                            ng2 * myblocks, cudaMemcpyDeviceToHost);
#endif //DEBUG_CUDA_TRANSFORMER
            if (real) {
        ...
        for (int m=0; m < myblocks; m++) {
            if (real) {
                if (self->interpolate)
                    bmgs_interpolate(self->k, self->skip, buf_cpu2 + m * ng2,
                                     bc->size2, out_cpu + m * out_ng,
                                     buf2_cpu + m * MAX(out_ng, ng2));
                else
                    bmgs_restrict(self->k, buf_cpu2 + m * ng2,
                                  bc->size2, out_cpu + m * out_ng,
                                  buf2_cpu + m * MAX(out_ng, ng2));
            } else {
                if (self->interpolate)
                    bmgs_interpolatez(
                            ...
                            bc->size2,
                            (double_complex*) (out_cpu + m * out_ng),
                            (double_complex*) (buf2_cpu + m
                                               * MAX(out_ng, ng2)));
                else
                    bmgs_restrictz(
                            self->k,
