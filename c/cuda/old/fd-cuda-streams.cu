...
__global__ void bmgs_fd_cuda_kernel(const bmgsstencil* s,const double* a,double* b,long nend0,long nend1,long nend2)
{
  ...
  //  long n[3]={s->n[0],s->n[1],s->n[2]};
  //  long j[3]={s->j[0],s->j[1],s->j[2]};
  __shared__ long n[3];
  __shared__ long j[3];
  ...
  */
  //  a+=threadIdx.x+blockIdx.x*(j[1]+n[1]*(j[2]+n[2]));
  //b+=threadIdx.x+blockIdx.x*n[1]*n[2];
  //  a+=threadIdx.x+0*(j[1]+n[1]*(j[2]+n[2]));
  //b+=threadIdx.x+0*n[1]*n[2];
  //  for (i0 = blockIdx.x; i0 < n[0]; i0+=gridDim.x) {
  for (i0 = 0; i0 < nend0; i0++) {
    //  for (int i0 = i0s; i0 < i0e; i0++) {
    // aa=a+blockIdx.y*(j[2]+n[2]);
    //bb=b+blockIdx.y*n[2];
    //    for (i1 = blockIdx.y; i1 < n[1]; i1+=gridDim.y) {
    for (i1=blockIdx.y*blockDim.y+threadIdx.y; i1 < nend1; i1+=blockDim.y*gridDim.y) {
      //    for (int i1 = i1s; i1 < i1e; i1++) {

      //     aa=a+i1*(j[2]+n[2]);
      //bb=b+i1*n[2];
      for (i2=blockIdx.x*blockDim.x+threadIdx.x; i2 < nend2; i2+=blockDim.x*gridDim.x) {
	//      for (i2 =threadIdx.x; i2 < n[2]; i2+=blockDim.x) {
	aa=a+i0*(j[1]+n[1]*(j[2]+n[2]))+i1*(j[2]+n[2])+i2;
	bb=b+i0*n[1]*n[2]+i1*n[2]+i2;
	x = aa[offsets[0]] * coefs[0];
	for (c = 1; c < ncoefs; c++){	  
	  x += aa[offsets[c]] * coefs[c];
...
fprintf(stdout,"pitch %zd\n",bdev.pitch);
  ...
  */
  ...
  for (int i=0;i<nstreams;i++){
    ...
    bmgs_fd_cuda_kernel<<<dimGrid, dimBlock, 0,streams[i]>>>(sdev,adevp+(s->j[0]+s->j[1]+s->j[2])/2,bdevp,MIN(nstep0,s->n[0]-i0),s->n[1],s->n[2]);
  }
  for (int i=0;i<nstreams;i++){
    i0=i*nstep0;
    bp=b+i0*s->n[1]*s->n[2];
