...
__global__ void bmgs_fd_cuda_kernel(int ncoefs,int ncoefsxy,const double* a,double* b)
{

  if (blockIdx.x>=r_n[0]) return;  
  ...
  a+=blockIdx.x*(r_j[1]+r_n[1]*(r_j[2]+r_n[2]))+i1*(r_j[2]+r_n[2]);
  b+=blockIdx.x*r_n[1]*r_n[2]+i1*r_n[2];
...
#pragma unroll 13
  ...
  for (i2=threadIdx.x+blockDim.x; i2 < rbm; i2+=blockDim.x) {    
    
#pragma unroll 3
    for (i=threadIdx.y;i<c_jb[1]+blodimy;i+=blodimy){
      if (threadIdx.x<r_j[2]){
	acache[i][threadIdx.x]=acache[i][threadIdx.x+blockDim.x];
      }
    }
    __syncthreads();  	  
...
#pragma unroll 13
  ...
  int minu=r_n[2]-rbm;
  if ((minu>0) && (threadIdx.x<minu+r_j[2])){
    i2=rbm+threadIdx.x;
    aa=a+i2-r_j[2]/2-r_j[1]/2;    
    if (rbm>0){
#pragma unroll 3
...
#pragma unroll 3
      for (i=threadIdx.y;i<c_jb[1]+blodimy;i+=blodimy){
	acache[i][threadIdx.x]=*(aa);
	if (threadIdx.x<r_j[2]+minu-blockDim.x)
	  acache[i][threadIdx.x+blockDim.x]=*(aa+blockDim.x);
	aa+=blodimy*(r_j[2]+r_n[2]);
...
__global__ void bmgs_fd_cuda_kernelz(int ncoefsz,const double* a,double* b)
{

   if (blockIdx.x>=r_n[2]) return;  
  int i1=blockIdx.y*blockDim.y+threadIdx.y;
  if (i1>=r_n[1]) return;  
  
  __shared__ double acachez[ACACHE_SIZEZ];
  int i,i2,c/*,xind,yind,xsize,ysize,i0c*/;
  double x;
  const double *aa,*aacachez/*,*aaa*/;
  /*double *bb,*bbb*/;
  int rbm=BLOCK_SIZEX*(r_n[2]/BLOCK_SIZEX);
  int blodimy=MIN(blockDim.y,r_n[1]-blockIdx.y*blockDim.y);

  a+=blockIdx.x+i1*(r_j[2]+r_n[2]);
  b+=blockIdx.x+i1*r_n[2];
  aacachez=&acachez[0]+threadIdx.x+c_jb[0]/2;

  for (int i0=threadIdx.x; i0 < r_n[0]; i0+=blockDim.x) {    
    aa=a+i0*(r_j[1]+r_n[1]*(r_j[2]+r_n[2]))-r_j[0]/2;    

    acachez[threadIdx.x]=*(aa);
    if (threadIdx.x<c_jb[0])
      acachez[threadIdx.x+blockDim.x]=*(aa+(r_j[1]+r_n[1]*(r_j[2]+r_n[2]))*blockDim.x);
    __syncthreads();  	  
    x = 0.0;    
#pragma unroll 13
    for (c = 0; c < ncoefsz; c++){
      x += aacachez[c_offsetsz[c]] * c_coefsz[c];
    }	
					//    aa=a+i0*(r_j[1]+r_n[1]*(r_j[2]+r_n[2]);
    b[i0*r_n[1]*r_n[2]]+= x;
  }
}
...
extern "C" {
  ...
  double bmgs_fd_cuda(const bmgsstencil* s, const double* a, double* b)
  {
	...
	offsetsz[ncoefsz]=s->offsets[i]/((jb[1]+s->n[1])*(jb[2]+s->n[2]));
    ...
    */
    ...
    bmgs_fd_cuda_kernel<<<dimGrid, dimBlock, 0>>>(ncoefs,ncoefsxy,adevp+(s->j[0]+s->j[1]+s->j[2])/2,bdevp);
    cudaThreadSynchronize();  
    dim3 dimBlockz(BLOCK_SIZEX,BLOCK_SIZEY); 
    dim3 dimGridz(s->n[2],gridy);
    bmgs_fd_cuda_kernelz<<<dimGridz, dimBlockz, 0>>>(ncoefsz,adevp+(s->j[0]+s->j[1]+s->j[2])/2,bdevp);
    cudaThreadSynchronize();  
