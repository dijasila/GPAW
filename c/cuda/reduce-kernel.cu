...
__global__ void INNAME(reduce_kernel)(const Tcuda *g_idata1,const Tcuda *g_idata2, Tcuda *g_odata,Tcuda *results, unsigned int n,unsigned int block_in,int block_out,int nvec)
{
  ...
  while (i < n)    {         
    IADD(mySum,INFUNC(g_idata1[i+block_in*i_vec],g_idata2[i+block_in*i_vec]));
    // ensure we don't read out of bounds 
    if (i +REDUCE_THREADS < n) 
      IADD(mySum,INFUNC(g_idata1[i+block_in*i_vec+REDUCE_THREADS],
  ...
  if (tid == 0) g_odata[blockIdx.x+block_out*i_vec] = Zcuda(sdata)[0];
  
  if (gridDim.x==1){
    __shared__ bool amLast;
    __threadfence();
    if (tid == 0) {
      unsigned int ticket = atomicInc(&INNAME(retirementCount), gridDim.y);
