...
#define XC_MGGA_X_OTPSS          207 /* oTPSS exchange */
#define NMIN   1.0E-10

#define   _(is, x)   [3*is + x]

/************************************************************************
 Semi-empirically optimized version of TPSS exchange.
...
const XC(func_info_type) XC(func_info_mgga_x_otpss) = {
  XC_MGGA_X_OTPSS,
  XC_EXCHANGE,
  "oTPSS",
  XC_FAMILY_MGGA,
...
{
  p->info = &XC(func_info_mgga_x_otpss);
...
/* some parameters */
static FLOAT b=3.43, c=0.75896, e=0.165, kappa=0.778, mu=0.41567;


/* This is Equation (7) from the TPSS paper and its derivatives */
static void 
x_otpss_7(FLOAT p, FLOAT alpha, FLOAT *qb, FLOAT *dqbdp, FLOAT *dqbdalpha)
{
  /* Eq. (7) */
...
static void
x_otpss_10(FLOAT p, FLOAT alpha, FLOAT *x, FLOAT *dxdp, FLOAT *dxdalpha)
{
  ...
  /* Equation 7 */
  x_otpss_7(p, alpha, &qb, &dqbdp, &dqbdalpha);
...
static void 
x_otpss_para(XC(mgga_type) *pt, FLOAT *rho, FLOAT sigma, FLOAT tau_,
             FLOAT *energy, FLOAT *dedd, FLOAT *vsigma, FLOAT *dedtau)
{
  ...
  /* get Eq. (10) */
  x_otpss_10(p, alpha, &x, &dxdp, &dxdalpha);
...
void 
XC(mgga_x_otpss)(XC(mgga_type) *p, FLOAT *rho, FLOAT *sigma, FLOAT *tau,
                FLOAT *e, FLOAT *dedd, FLOAT *vsigma, FLOAT *dedtau)
{
  if(p->nspin == XC_UNPOLARIZED){
    FLOAT en;
    x_otpss_para(p, rho, sigma[0], tau[0], &en, dedd, vsigma, dedtau);
    *e = en/(rho[0]+rho[1]);
  }else{ 
    ...
    *e = 0.0;
    ...
    x_otpss_para(p, rhoa, 4*sigma[0], 2.0*tau[0], &e2na, &(dedd[0]), &(vsigmapart[0]), &(dedtau[0]));
    x_otpss_para(p, rhob, 4*sigma[2], 2.0*tau[1], &e2nb, &(dedd[1]), &(vsigmapart[2]), &(dedtau[1]));
