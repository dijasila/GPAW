...
#define XC_MGGA_C_OTPSS          208 /* oTPSS correlation */
#define NMIN   1.0E-10

/************************************************************************
 Semi-empirically optimized version of TPSS correlation.
...
const XC(func_info_type) XC(func_info_mgga_c_otpss) = {
  XC_MGGA_C_OTPSS,
  XC_CORRELATION,
  "Goerigk and Grimme",
  XC_FAMILY_MGGA,
...
{
  p->info = &XC(func_info_mgga_c_otpss);

  p->gga_aux1 = (XC(gga_type) *) malloc(sizeof(XC(gga_type)));
  XC(gga_init)(p->gga_aux1, XC_GGA_C_PBE_OTPSS, p->nspin);

  if(p->nspin == XC_UNPOLARIZED){
    p->gga_aux2 = (XC(gga_type) *) malloc(sizeof(XC(gga_type)));
    XC(gga_init)(p->gga_aux2, XC_GGA_C_PBE_OTPSS, XC_POLARIZED);
  }
}
...
/* Equation (14) */
static void
c_otpss_14(FLOAT csi, FLOAT zeta, FLOAT *C, FLOAT *dCdcsi, FLOAT *dCdzeta)
{
...
/* Equation (12) */
static void c_otpss_12(XC(mgga_type) *p, FLOAT *rho, FLOAT *sigma, 
		 ...
		 FLOAT *e_PKZB, FLOAT *de_PKZBdd, FLOAT *de_PKZBdsigma, FLOAT *de_PKZBdz)
{
  ...
  if(p->nspin == XC_UNPOLARIZED){   
    ...
    c_otpss_14(csi, zeta, &C, &dCdcsi, &dCdzeta);
...
void 
XC(mgga_c_otpss)(XC(mgga_type) *p, FLOAT *rho, FLOAT *sigma, FLOAT *tau,
	    FLOAT *energy, FLOAT *dedd, FLOAT *vsigma, FLOAT *dedtau)
{
  ...
  /* Equation (12) */
  c_otpss_12(p, rho, sigma, dens, zeta, z,
