...
#define XC_GGA_C_PBE_OTPSS    138 /* PBE for oTPSS                                  */
...
static void gga_c_pbe_init(void *p_)
{
  ...
  case XC_GGA_C_XPBE:
    ...
    break;
  case XC_GGA_C_PBE_OTPSS:
  case XC_GGA_C_PBE_REVTPSS:
  case XC_GGA_C_PBE_SOL:
  default: /* the original PBE */
    gamm[0] = gamm[1] = gamm[3] = gamm[4] = (1.0 - log(2.0))/(M_PI*M_PI);
    break;
  }  
}
	...
	FLOAT *d2phi, FLOAT *d2phit, FLOAT *d2phiA, FLOAT *d2t2, FLOAT *d2tA, FLOAT *d2A2)
{
  ...
  *d2t2   = gamm[func]*phi3*(d2f2dt2*(1.0 + f2) - df2dt*df2dt)/((1.0 + f2)*(1.0 + f2));
  ...
  d2f1dtA = 4.0*t*t2;
  d2f2dtA = beta[func]/(gamm[func]*f3*f3) * (d2f1dtA - 2.0*df1dt*(f1 + A*df1dA)/f3);
	  ...
	  FLOAT *v2rho2, FLOAT *v2rhosigma, FLOAT *v2sigma2)
{
  ...
  case XC_GGA_C_PBE_OTPSS:   func = 4; break;
...
const XC(func_info_type) XC(func_info_gga_c_pbe_revtpss) = {
  ...
  XC_CORRELATION,
  "Perdew, Burke & Ernzerhof for revTPSS",
  XC_FAMILY_GGA,
...
const XC(func_info_type) XC(func_info_gga_c_pbe_otpss) = {
  XC_GGA_C_PBE_OTPSS,
  XC_CORRELATION,
  "Goerigk and Grimme for oTPSS",
  XC_FAMILY_GGA,
