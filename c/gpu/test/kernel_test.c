...
int check_result(int n0, int n1, int n2, int blocks, double *b,
                 double *b_cuda)
{
    int e_i = 0;
    int bsize = n0 * n1 * n2;
            ...
            for (int i1=0; i1 < n1; i1++) {
                for (int i2=0; i2 < n2; i2++) {
                    ...
                    if (ee > 1e-7) {
                        if (e_i < 64) {
                            fprintf(stdout,
                                    "error [[%d,%d,%d],%d] er:%f cpu:%f gpu:%f\n",
...
int check_resultz(int n0, int n1, int n2, int blocks, cuDoubleComplex *b,
                  cuDoubleComplex *b_cuda)
{
    int e_i = 0;
    int bsize = n0 * n1 * n2;
    double error = 0;
    ...
    for (int k=0; k < blocks; k++) {
        for (int i0=0; i0 < n0; i0++) {
            for (int i1=0; i1 < n1; i1++) {
                for (int i2=0; i2 < n2; i2++) {
                    long i = i0 * n1 * n2 + i1 * n2 + i2;
                    double ee = sqrt((b[i].x - b_cuda[i].x)
                                     * (b[i].x - b_cuda[i].x)
                                     + (b[i].y - b_cuda[i].y)
                                     * (b[i].y - b_cuda[i].y));
                    if (ee > 1e-14) {
...
int calc_test_fd(int n1, int n2, int n3, int blocks)
{
    ...
    flops = t1.tv_sec + t1.tv_usec / 1000000.0 - t0.tv_sec
          - t0.tv_usec / 1000000.0;
    flops2 = t2.tv_sec + t2.tv_usec / 1000000.0 - t1.tv_sec
          - t1.tv_usec / 1000000.0;
    printf("(%3d,%3d,%3d,%3d) \t %f\t %f\t %3.1f", s.n[0], s.n[1], s.n[2],
           blocks, ntimes * blocks * bsize / (1000000.0 * flops),
           ntimes * blocks * bsize / (1000000.0 * flops3), flops / flops3);
...
int calc_test_dotu(int n1, int n2, int n3, int blocks)
{
    ...
    gpaw_cudaSafeCall(
            cudaMalloc((void**) &a_gpu, sizeof(double) * asize * blocks));
    gpaw_cudaSafeCall(
            cudaMalloc((void**) &b_gpu, sizeof(double) * bsize * blocks));
    gpaw_cudaSafeCall(
            cudaMemcpy(a_gpu, a, sizeof(double) * asize * blocks,
                       cudaMemcpyHostToDevice));
    gpaw_cudaSafeCall(
            cudaMemcpy(b_gpu, b, sizeof(double) * bsize * blocks,
                       cudaMemcpyHostToDevice));
    gpaw_cudaSafeCall(cudaGetLastError());

    mdotu_cuda_gpu(a_gpu, b_gpu, c_cuda, (int) asize, blocks);
    c_cuda[0] = cublasDdot(asize, (double*) a_gpu, inc, (double*) b_gpu, inc);
    ...
    for (int i=0; i < ntimes; i++) {
        for (int k=0; k < blocks; k++)
            c_cuda[k] = cublasDdot(asize, (double*) a_gpu + k * asize, inc,
                                   (double*) b_gpu + k * bsize, inc);
    }
    cudaThreadSynchronize();
    gettimeofday(&t2,NULL);
    for (int i=0; i < ntimes; i++)
    ...
    flops2 = t2.tv_sec + t2.tv_usec / 1000000.0 - t1.tv_sec
           - t1.tv_usec / 1000000.0;
    flops3 = t3.tv_sec + t3.tv_usec / 1000000.0 - t2.tv_sec
           - t2.tv_usec / 1000000.0;
    printf("(%3d,%3d,%3d,%3d) \t %f\t %f\t %f\t %3.1f\t %3.1f\t %3.1f",
            n[0], n[1], n[2], blocks,
            ntimes * blocks * bsize / (1000000.0 * flops),
            ntimes * blocks * bsize / (1000000.0 * flops2),
            ntimes * blocks * bsize / (1000000.0 * flops3),
            flops / flops2, flops / flops3, flops2 / flops3);
...
int calc_test_interpolate(int n1, int n2, int n3, int blocks)
{
    ...
    gettimeofday(&t1, NULL);
    for (int i=0; i < ntimes; i++)
        flops3 += bmgs_interpolate_cuda_cpu(2, skip, a, n, b_cuda, blocks);
    gettimeofday(&t2, NULL);
    ...
    flops2 = t2.tv_sec + t2.tv_usec / 1000000.0 - t1.tv_sec
           - t1.tv_usec / 1000000.0;
    printf("(%3d,%3d,%3d,%3d) -> (%3d,%3d,%3d,%3d) \t %f\t %f\t %3.1f",
            n[0], n[1], n[2], blocks, b_n[0], b_n[1], b_n[2], blocks,
            ntimes * blocks * bsize / (1000000.0 * flops),
            ntimes * blocks * bsize / (1000000.0 * flops3), flops / flops3);
...
int calc_test_restrict(int n1, int n2, int n3, int blocks)
{
    ...
    gettimeofday(&t1, NULL);
    for (int i=0; i < ntimes;i++)
        flops3 += bmgs_restrict_cuda_cpu(2, a_cuda, n, b_cuda, blocks);
    gettimeofday(&t2, NULL);
...
int calc_test_paste(int n1, int n2, int n3, int blocks)
{
    ...
    gettimeofday(&t1, NULL);
    for (int i=0; i < ntimes; i++)
        flops3 += bmgs_paste_cuda_cpu(a, ni, b_cuda, ni, c);
    gettimeofday(&t2, NULL);
    ...
    flops2 = t2.tv_sec + t2.tv_usec / 1000000.0 - t1.tv_sec
           - t1.tv_usec / 1000000.0;
    printf("(%3d,%3d,%3d,%3d) \t %f\t %f\t %f\t %3.1f",
            s.n[0], s.n[1], s.n[2], blocks,
            ntimes * blocks * bsize / (1000000.0 * flops),
            ntimes * blocks * bsize / (1000000.0 * flops3), flops/flops3);
...
int calc_test_relax(int n1, int n2, int n3)
{
    ...
    gettimeofday(&t1, NULL);
    for (int i=0; i < nrelax; i++)
        flops3 += bmgs_relax_cuda_cpu(relax_method, &s, a, b_cuda, src, w);
    gettimeofday(&t2, NULL);
    ...
    flops2 = t2.tv_sec + t2.tv_usec / 1000000.0 - t1.tv_sec
           - t1.tv_usec / 1000000.0;
    printf("(%3d,%3d,%3d) \t %f\t %f\t %3.1f", s.n[0], s.n[1], s.n[2],
            nrelax * bsize / (1000000.0 * flops),
            nrelax * bsize / (1000000.0 * flops3), flops / flops3);
...
int calc_test_fdz(int n1, int n2, int n3, int blocks)
{
    ...
    for (int i=0; i < ntimes; i++) {
        for (int k=0; k < blocks; k++)
            bmgs_fdz(&s, (double complex*) a + k * asize,
                     (double complex*) b + k * bsize);
    }
    gettimeofday(&t1, NULL);
    for (int i=0; i < ntimes;i++)
