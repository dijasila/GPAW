...
extern gpublasHandle_t _gpaw_gpublas_handle;
                               ...
                               int size_b, int size_c, int size_c_gpu)
{
    ...
    if (ph->size_a < size_a) {
        if (!ph->a)
            gpuFreeHost(ph->a);
        ph->size_a = size_a;
        gpuHostAlloc(&ph->a, sizeof(double) * ph->size_a);
    }
    if (ph->size_b < size_b) {
        if (!ph->b)
            gpuFreeHost(ph->b);
        ph->size_b = size_b;
        gpuHostAlloc(&ph->b, sizeof(double) * ph->size_b);
    ...
    if (ph->size_c < size_c) {
        if (!ph->c)
            gpuFreeHost(ph->c);
        ph->size_c = size_c;
        gpuHostAlloc(&ph->c, sizeof(double) * ph->size_c);
    }
    if (ph->size_c_gpu < size_c_gpu) {
        if (!ph->c_gpu)
            gpuFree(ph->c_gpu);
        ph->size_c_gpu = size_c_gpu;
        gpuMalloc(&ph->c_gpu, sizeof(double) * ph->size_c_gpu);
    }
}
                                  ...
                                  hybrid_params_t *ph)
{
    ...
    gpuMalloc(&a_gpu, sizeof(double) * k1 * m1);
    gpuMalloc(&b_gpu, sizeof(double) * k1 * n1);
    ...
    for (int i=0; i < times+1; i++) {
        ...
        gpublasSafeCall(
                gpublasDgemm(_gpaw_gpublas_handle, GPUBLAS_OP_N, GPUBLAS_OP_N,
        ...
        gpublasSafeCall(
                gpublasGetMatrixAsync(m2, k2, sizeof(double),
        ...
        gpublasSafeCall(
                gpublasSetMatrixAsync(m2, n2, sizeof(double),
    ...
    gpublasSafeCall(gpublasSetStream(_gpaw_gpublas_handle, 0));
    gpuFree(a_gpu);
    gpuFree(b_gpu);
}
                                  ...
                                  hybrid_params_t *ph)
{
    ...
    gpuMalloc(&a_gpu, sizeof(double) * k1 * n1);
    gpuMalloc(&b_gpu, sizeof(double) * k1 * n1);
    ...
    for (int i=0; i < times+1; i++) {
        ...
        gpublasSafeCall(
                gpublasDsyrk(_gpaw_gpublas_handle, GPUBLAS_FILL_MODE_UPPER,
                            GPUBLAS_OP_T, n1, k1, &alpha, (double*) a_gpu, lda1,
                            &beta, (double*) ph->c_gpu, ldc1));
        gpuEventRecord(ps->event_gpu[1], ph->stream[0]);
        /* data transfer device -> host */
        gpuEventRecord(ps->event_dtoh[0], ph->stream[1]);
        gpublasSafeCall(
                gpublasGetMatrixAsync(n2, k2, sizeof(double),
        ...
        gpublasSafeCall(
                gpublasSetMatrixAsync(n2, n2, sizeof(double),
                                   ...
                                   hybrid_params_t *ph)
{
    ...
    for (int i=0; i < times+1; i++) {
        ...
        gpuEventRecord(ps->event_gpu[0], ph->stream[0]);
        gpublasSafeCall(
                gpublasDsyr2k(_gpaw_gpublas_handle, GPUBLAS_FILL_MODE_UPPER,
                             GPUBLAS_OP_T, n1, k1, &alpha,
...
void gemm_gpu_hybrid(char transa, gpublasOperation_t transa_c,
                     int m, int n, int k,
                     ...
                     int real)
{
    ...
    if (!beta_null) {
        gpublasSafeCall(
                gpublasGetMatrixAsync(pg->m2, pg->n2,
                                     sizeof(double) * pg->ndouble,
                                     (void*) ((double*) c_gpu
    ...
    gpuEventRecord(pg->event_htod[0], ph->stream[1]);
    gpublasSafeCall(
            gpublasSetMatrixAsync(pg->m2, pg->n2,
                                 sizeof(double) * pg->ndouble,
                                 (void*) ph->c, ldc2,
                                 (void*) ((double*) c_gpu
                                     + n_off * ldc * pg->ndouble
...
void rk_gpu_hybrid(int n, int k,
                   ...
                   int real)
{
    ...
    gpublasSafeCall(gpublasSetStream(_gpaw_gpublas_handle, ph->stream[0]));
    gpuEventRecord(ps->event_gpu[0], ph->stream[0]);
    if (real) {
        gpublasSafeCall(
    ...
    gpuEventRecord(ps->event_htod[0], ph->stream[1]);
    gpublasSafeCall(
            gpublasSetMatrixAsync(ps->n, ps->n,
                                 sizeof(double) * ps->ndouble,
                                 (void*) ph->c, ldc,
    ...
    gpublasSafeCall(
            gpublasDaxpy(_gpaw_gpublas_handle, ps->n * ps->n * ps->ndouble,
                &alpha_, (double*) ph->c_gpu, 1, (double*) c_gpu, 1));
    gpuEventRecord(ps->event_gpu[3], 0);
}


void r2k_gpu_hybrid(int n, int k,
                    Py_complex alpha, void *a_gpu, int lda,
                    ...
                    void *c_gpu, int ldc, int real)
{
    ...
    gpublasSafeCall(
            gpublasGetMatrixAsync(ps2->k2, n,
    ...
    gpublasSafeCall(
            gpublasSetMatrixAsync(ps2->n, ps2->n,
                                 sizeof(double) * ps2->ndouble,
                                 (void*) ph->c, ldc,
                                 (void*) ((double*) ph->c_gpu),
                                 ldc, ph->stream[1]));
    ...
    gpublasSafeCall(
            gpublasDaxpy(_gpaw_gpublas_handle,
